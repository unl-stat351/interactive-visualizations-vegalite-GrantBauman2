...
> Instead of saturation slider we could add sliders for RGB and HLS depending on which one you have selected. These could have max and min ranges to show only certain color ranges like purples or greens.

Is your proposed graphic supported by vega-lite? Why or why not? (You can provide links to documentation/stackOverflow)

> Yes, It wouldn't be much different than what we are doing below in my mind.
...
// filter out points based on saturation
filtered2 = colors.filter(d => (d.s <= maxSat2) && (d.s >= minSat2))
// Create an x variable that's conditional on what is selected,
// changing both the values and the label accordingly
